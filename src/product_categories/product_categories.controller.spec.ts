...
describe('ProductCategoriesController', () => {
  ...
  describe('GET /product-categories/:id', () => {
    ...
    it('should return a NotFoundException if category does not exist', async () => {
      mockService.getProductCategoryById
      .mockRejectedValueOnce(new NotFoundException('Product category not found.'));

      await expect(controller.getProductCategoryById(-1))
      .rejects
      .toThrow(
  ...
  describe('PATCH /product-categories/:id', () => {
    ...
    it('should return a NotFoundException if category does not exist', async () => {
      ...
      mockService.updateProductCategory
      .mockRejectedValueOnce(new NotFoundException('Product category not found.'));

      await expect(controller.updateProductCategory(-1, dto))
      .rejects
      .toThrow(
        new NotFoundException('Product category not found.')
      );
  ...
  describe('DELETE /product-categories/:id', () => {
    ...
    it('should return a NotFoundException if category does not exist', async () => {
      mockService.deleteProductCategory
      .mockRejectedValueOnce(new NotFoundException('Product category not found.'));

      await expect(controller.deleteProductCategory(-1))
      .rejects
      .toThrow(
        new NotFoundException('Product category not found.')
      );
